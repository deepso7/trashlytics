...
- The `effect` peer range widened from the exact `4.0.0-rc.113` pin to `>=4.0.0-rc.113 <5` (CI installs each in-range version under test as releases ship), and the package now ships a `LICENSE` and declares `engines.node` `>=20`.
